...
<!-- Basic datatable -->
	<div class="panel panel-flat">
		<div class="panel-heading">
			<h5 class="panel-title">Data Katalog</h5>
		...
		<table class="table datatable-basic">
			...
			<tbody>
				...
				<tr>
					...
					<td>{{ $data->desc_katalog }}</td>
					<td>{{ $data->created_at }}</td>
					<td>{{ $data->updated_at }}</td>
					<td>{{ $data->updated_by }}</td>
					<td>
						<a href="{{ route('data.katalog.edit', $data->id) }}">
							<button type="button" class="btn btn-info btn-xs"><i class="icon-pencil7"></i> Edit</button>
						</a>
					</td>
					<td>
						<a onclick="return confirm('Apakah anda benar ingin menghapus data ini?')" href="{{ route('data.katalog.delete', ['id'=>$data->id]) }}">
